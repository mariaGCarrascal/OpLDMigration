...
class Approachspeeddistancecalculation{
  ...
  double call() {
    ...
    try {
      ...
      //Operation for adjustement result, falta obtener el valor indice del airspeedData
      double selectedVref = double.tryParse(airspeedRef ?? '') ?? 0.0;
      double approachSpeedAdjustment = double.tryParse(airspeedData['$selectedautobrake'] ?? '') ?? 0.0;
      double baseLineRefVref = double.tryParse(baseDetails!['refDltaVref'] ?? '') ?? 0.0;
      double perHowManyAirspeedUnitsAboveVref = double.tryParse(baseDetails!['perHowManyAirspeedUnitsAboveVref'] ?? '') ?? 0.0;
